...
BEGIN
  ...
  SELECT opt_in_responder, topic_tags
    INTO v_opt_in, v_user_tags
  FROM "public"."users"
  WHERE id = v_uid;

  IF v_opt_in IS NOT TRUE THEN
    RAISE EXCEPTION 'opt_in_required';
  END IF;
...
GRANT EXECUTE ON FUNCTION "public"."claim_thread_for_responder"(uuid) TO authenticated;
